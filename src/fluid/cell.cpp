...
void cell::update(double delta_t) {
    send_contents(pre_update_state.velocity(), delta_t);
}
...
void cell::send_contents(double velocity, double delta_t) {
    ...
    double volume_displaced = dimensions_.area * fabs(velocity) * delta_t;
    double volume_sum = 0;
    std::shared_ptr<cell> root_cell = shared_from_this();
    std::shared_ptr<cell> next_cell = nullptr;

    while (true) {
        std::vector<std::shared_ptr<cell>>& neighbors = (go_backwards ? root_cell->neighbors_low : root_cell->neighbors_high);
